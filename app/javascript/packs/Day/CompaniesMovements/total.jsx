...
import {accountsSaldo, sumMovsByCurrency, DepositInfo} from "./oneCompany"
...
export class Total extends React.Component{
	...
	render(){
		...
		//calculate saldo on all accounts by all selected companies 
		...
		//calculate saldo on deposit accounts (acc_type_id==2) by all selected companies and send them to buffer as 'total:
		const all_deposits_on_date = accountsSaldo(
			voc.accsList.filter(a => a.acc_type_id == '2'&& companiesSelectedIds.includes(a.company_id) ),
			movementsCounted
		) 
